...
parser.add_argument('-cons', '--consensus', type=str, default='none', help="the consensus mechanism to use, -pow takes effect only if this option is set to 'pow'")
...
if __name__=="__main__":
	...
	if args['resume_path']:
		...
		if args['consensus'] == 'poa' and miners_needed != 0:
			sys.exit("ERROR: When using PoA, a miner is chosen among the validators, please specify 0 for miners")

		if num_devices < workers_needed + validators_needed + miners_needed:
		...
		# 12. build logging files/database path
		# create log files
	...
	for comm_round in range(latest_round_num + 1, args['max_num_comm']+1):
		...
		# first choose validators based on result of previous round, i.e choose best workers of previous round as validators for this round
		...
		for worker_iter, worker in enumerate(workers_this_round):
			...
			if mining_consensus != 'PoA' and worker.online_switcher():
		...
		for validator_iter, validator in enumerate(validators_this_round):
			...
			if  mining_consensus != 'PoA' and validator.online_switcher():
			...
			if args['miner_acception_wait_time']:
				...
				for worker_iter, worker in enumerate(associated_workers):
					# worker = associated_workers[worker_iter]
					if not worker.return_idx() in validator.return_black_list():
						print(f'worker {worker_iter+1}/{len(associated_workers)} of validator {validator.return_idx()} is doing local updates')	 
						if worker.online_switcher():
		...
		for validator_iter, validator in enumerate(validators_this_round):
			...
			if final_transactions_arrival_queue:
				...
				for (arrival_time, unconfirmmed_transaction) in final_transactions_arrival_queue:
					if validator.online_switcher():
						# validation won't begin until validator and validation(worker transactions will be queued)
