...
class Device:
	def __init__(self, idx, assigned_train_ds, assigned_test_dl, local_batch_size, learning_rate, loss_func, opti, network_stability, net, dev, miner_acception_wait_time, miner_accepted_transactions_size_limit, validator_threshold, pow_difficulty, even_link_speed_strength, base_data_transmission_speed, even_computation_power, is_malicious, noise_variance, check_signature, not_resync_chain, malicious_updates_discount, knock_out_rounds, lazy_worker_knock_out_rounds, mining_consensus):
		...
		self.cons = mining_consensus.lower()
	...
	def verify_miner_transaction_by_signature(self, transaction_to_verify, miner_device_idx):
		...
		if self.check_signature:
			...
			if hash == hashFromSignature:
				print(f"A transaction recorded by miner {miner_device_idx} in the block is verified by {self.return_idx()}!")
				return True
			else:
	...
	def poa_process_block(self, block_to_process, log_files_folder_path, conn, conn_cursor, when_resync=False):
		# TO-DO: flag here the devices as malicious when condition matched and delete all related to rewards here
		# collect usable updated params, malicious nodes identification, get rewards and do local udpates
		processing_time = time.time()
		if not self.online_switcher():
			print(f"{self.role} {self.idx} goes offline when processing the added block. Model not updated and rewards information not upgraded. Outdated information may be obtained by this node if it never resyncs to a different chain.") # may need to set up a flag indicating if a block has been processed
	...
	def worker_local_update(self, rewards, log_files_folder_path_comm_round, comm_round, local_epochs=1):
		...
		for epoch in range(local_epochs):
			...
			with open(f"{log_files_folder_path_comm_round}/worker_{self.idx}_{is_malicious_node}_local_updating_accuracies_comm_{comm_round}.txt", "a") as file:
				file.write(f"{self.return_idx()} epoeh_{epoch+1} {self.return_role()} {is_malicious_node}: {self.validate_model_weights(self.net.state_dict())}\n")
	...
	def calculate_global_update(self, block=None):
		...
		# local_update_params_potentially_to_be_used = block.return_local_updates()	
		# # filter local_params
		# global_params = {}
		# local_params_by_benign_workers = []
		# for (worker_device_idx, local_params) in local_update_params_potentially_to_be_used:
	...
	def verify_validator_transaction(self, transaction_to_verify):
		if self.computation_power == 0:
			...
			if transaction_validator_idx in self.black_list:
				print(f"{transaction_validator_idx} is in blacklist. Trasaction won't get verified.")
				return False, None
			verification_time = time.time()
			if self.check_signature:
				transaction_before_signed = copy.deepcopy(transaction_to_verify)
...
#		self.block_to_add = None
	...
	def validator_reset_vars_for_new_round(self):
		...
		# self.aggregated_accuracy_gains.clear()
	...
	# def add_received_post_validation_transaction(self, transaction_to_add):
	# 	if all(transaction_to_add['worker_device_idx']!=worker['worker_device_idx'] for worker in self.received_post_validation_transactions):
	# 		self.received_post_validation_transactions.append(transaction_to_add)
